    ...
    def __init__(
        ...
        if use_norm:
            self.norm1 = nn.GroupNorm(
                num_groups=32, num_channels=out_c, eps=1e-6, affine=True
            )
...
class Adapter_XL(nn.Module):
    def __init__(
        self,
        in_channels=[1280, 640, 320],
        out_channels=[1280, 1280, 640],
        nums_rb=3,
        ksize=3,
        sk=True,
        use_conv=False,
                ...
                if i == 0:
                    ...
                    self.body.append(
                        ResnetBlock(
                            ...
                            sk=sk,
                            use_conv=use_conv,
                            enable_timestep=enable_timestep,
    ...
    def forward(self, x, t=None):
        ...
        output_size = (
            ...
            (x[0].shape)[2] * 4,
            (x[0].shape)[3] * 4,
        )  # last CA layer output should suit to the input size  CSR
...
if __name__ == "__main__":
    ...
    x = [
        torch.randn(4, 1280, 32, 32).cuda(),
        torch.randn(4, 640, 64, 64).cuda(),
        torch.randn(4, 320, 64, 64).cuda(),
    ]
